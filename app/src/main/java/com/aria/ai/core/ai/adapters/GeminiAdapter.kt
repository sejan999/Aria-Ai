 ...
 *
 * No model id is hardcoded: the adapter asks [GeminiModelDiscovery] for the
 * catalogue and lets it rank the best model for the task at hand, caching the
 * outcome for 24 hours through [ModelResolver]. Text turns resolve
 * [TaskType.CHAT]; a turn carrying an image resolves [TaskType.VISION], so
 * screen understanding automatically gets a multimodal model.
 *
 * The key travels in the `x-goog-api-key` header instead of the query string so
 * it can never leak through a logged/echoed URL.
 */
@Singleton
class GeminiAdapter @Inject constructor(
    ...
    override fun streamChat(messages: List<Message>, options: ChatOptions): Flow<ChatChunk> = flow {
        ...
        emitAll(AiHttp.streamSse(client, url, headers, payload) { data -> parseChunk(data, model) })
    }
